...
- La app usa los artefactos *_light.parquet por defecto.
- El parquet completo queda como fallback explícito.
- El scope se limita a Animalia + Plantae, coherente con el entregable.
"""
...
# Columnas que usa la app. Si el parquet trae más columnas, no las cargamos en memoria.
...
def _read_parquet_selected_columns(file_path: str | Path, expected_columns: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        # Fallback compatible con tests o entornos sin pyarrow schema.
        pass
...
def _normalize_conservation_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Asegura columnas de conservación coherentes con IUCN v4."""
    result_df = df.copy()

    if "iucn_category" not in result_df.columns:
        result_df["iucn_category"] = result_df.get("conservation_status", "NO_DATA")

    if "iucn_status_label" not in result_df.columns:
        result_df["iucn_status_label"] = result_df.get("conservation_category", "Sin datos IUCN")

    if "conservation_status" not in result_df.columns:
        result_df["conservation_status"] = result_df["iucn_category"].fillna("NO_DATA")

    if "conservation_category" not in result_df.columns:
        result_df["conservation_category"] = result_df["iucn_status_label"].fillna("Sin datos IUCN")
    ...
    if "iucn_is_official" not in result_df.columns:
        result_df["iucn_is_official"] = result_df["conservation_source"].fillna("").astype(str).eq("IUCN Red List")
...
def load_encyclopedia() -> pd.DataFrame:
    ...
    try:
        ...
        # Fallback defensivo: si el light artifact no existe todavía, usar completo.
...
def load_occurrence_points() -> pd.DataFrame:
    ...
    Usa el artifact light por defecto porque el mapa está dentro de un expander y no
    necesita todos los puntos del dataset completo para la demo.
    """
    if is_offline_mode():
