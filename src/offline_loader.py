...
class ArtifactMode(str, Enum):
    ...
    OFFLINE_LOCAL = "offline_light"
    # Compatibility alias for older patches/tests that used this name.
    OFFLINE_LIGHT = "offline_light"


MODE_LABELS: dict[ArtifactMode | str, str] = {
...
def expected_offline_files(target_dir: str | Path | None = None) -> list[Path]:
    """Return all expected local offline artifact paths."""
    return list(offline_artifact_paths(target_dir).values())


def missing_offline_files(target_dir: str | Path | None = None) -> list[Path]:
    """Return expected local paths that are not available yet."""
...
def missing_offline_artifacts(target_dir: str | Path | None = None) -> list[str]:
    ...
    return [path.name for path in missing_offline_files(target_dir)]
...
def list_offline_artifacts(target_dir: str | Path | None = None) -> list[Path]:
    ...
    return [path for path in expected_offline_files(target_dir) if path.exists()]
...
def normalize_artifact_mode(mode: ArtifactMode | str | None) -> ArtifactMode:
    """Normalize legacy strings and enum values to a stable ArtifactMode."""
    if mode is None:
        return get_default_artifact_mode()
    raw = str(mode.value if isinstance(mode, ArtifactMode) else mode).strip().lower()
    if raw == "offline_local":
        raw = "offline_light"
    try:
        return ArtifactMode(raw)
    except ValueError:
        return get_default_artifact_mode()
...
def describe_artifact_mode(mode: ArtifactMode | str) -> str:
    ...
    resolved_mode = normalize_artifact_mode(mode)
    descriptions = {
